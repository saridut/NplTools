...
character(len=:), allocatable :: fn_top
...
help_text = [&
'USAGE                                                                       ',&
'  bin2txt --fn_top <file> --fn_fns <file> --read_markers                    ',&
'    --col_nums <list of ints> --coord_desc <list of strings>                ',&
'    --atm_names <list of strings>  --fn_out <string> --frame <int>          ',&
'    --to <int> --stp <int>                                                  ',&
'                                                                            ',&
'DESCRIPTION                                                                 ',&
...
'                                                                            ',&
'  --fn_fns <file>                                                           ',&
'    File containing the names of the trajectory files and (possibly) offsets',&
'    for each frame (referred to as markers). If offsets are present set the ',&
'    option `--read_markers`. If --read_markers is not set, this file will be',&
'    overwritten with calculated values of offsets.                          ',&
'                                                                            ',&
'  --read_markers                                                            ',&
'    Flag to indicate whether the --fn_fns includes frame offsets.           ',&
'                                                                            ',&
...
do iframe = ifrm_beg, ifrm_end, ifrm_stp
    ...
    if (ierr /= 0) exit
    oframe = oframe + 1
    if (mod((iframe-ifrm_beg),10)==0) then
        write(*,'(a,1x,i0,1x,i0)') 'Frame', iframe, 'time step', nts
    end if
